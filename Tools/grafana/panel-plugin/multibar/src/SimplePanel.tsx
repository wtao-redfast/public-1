...
class TimedBarsItem {
  ...
  total_storypoints: number = 0;
  completed_tickets: number = 0;
  completed_storypoints: number = 0;
}

const colorPalette = ['#659954', '#D19E2A', '#59B7C7', '#D6672C', '#cddc49', '#cb7e94', '#e94b30', '#fee659', '#a1cfdd', '#33cee1', '#98ffc3', '#a3abd0'];
...
export const SimplePanel: React.FC<Props> = ({ options, data, width, height }) => {
  ...
  useEffect(() => {
    ...
    // @ts-ignore:
    let tooltip = d3.select(refTooltip.current)
      .style('opacity', 0);
    
    // @ts-ignore:
    let svg = d3.select(refD3.current)
      .attr('width', width)
          ...
          barNames.map((name, index) => {
            return (
              <div style={{display: 'flex', alignItems: 'center'}}> 
                ...
                {/* @ts-ignore */}
                <sp key={{index}} style={{marginRight: '15px', fontSize: '12px'}}>{name}</sp>
              </div>)
          })
        }
      </div>
